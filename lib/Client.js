...
function Client() {
  ...
  this._client.on('connect', function() {
    ...
    // MARKCODE MOD: problem with SELECT CONNECTION_ID() is sphinxql does not support this call so maybe stick with mysql_thread_id() instead.
    // OR set connection id = 1 with added config paramater if ( isSphinx === true ) { ...
    self.query('SHOW STATUS', true, true)
  ...
  if (this._curResults === undefined && this.connected) {
    ...
    if (this._queries.length) {
      ...
      this._pinger = setTimeout(function ping() {
        // MARKCODE: DO 0 throws sphinx error, invalid command; so replaces with SHOW META.
		//self.query('DO 0', true, true);
        self.query('SHOW META', true, true);
      }, this.pingInterval);
    }
  }
};
